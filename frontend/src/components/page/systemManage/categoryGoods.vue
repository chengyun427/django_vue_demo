  ...
  <div class="menuTreeRight">
     ...
     <div class="mar-top-10">
          <el-form ref="menuDetail" :inline="true"  :model="menuDetail" label-width="100px">
            <el-form-item label="分类名称">
              <el-input v-model="menuDetail.categoryName" readonly></el-input>
            </el-form-item>
            <el-form-item label="上级分类编码">
              <el-input v-model="menuDetail.categoryParentId" readonly></el-input>
            </el-form-item>
            <el-form-item label="树状查询码">
              <el-input v-model="menuDetail.categoryTreeCode" readonly></el-input>
            </el-form-item>
            <el-form-item label="排序">
              <el-input v-model="menuDetail.sortNo" readonly></el-input>
            </el-form-item>
            <el-form-item label="备注">
              <el-input v-model="menuDetail.categoryRemark" readonly></el-input>
            </el-form-item>
          </el-form>
      </div>
  </div>
    ...
    <el-dialog title="新增菜单" :visible.sync="dialogMenuAdd" width="30%" :close-on-click-modal="false">
      <el-form :model="addMenuList" :rules="rulesAddMenu" ref="addMenuList" label-width="100px">
        ...
        <el-form-item label="上级分类编码" prop="categoryParentId"  class="mar-top-20">
            <el-input  v-model="addMenuList.categoryParentId" clearable class="inputWidth" ></el-input>
        </el-form-item>
        <el-form-item label="树状查询码"   prop="categoryTreeCode"   class="mar-top-20" >
          <el-input  v-model="addMenuList.categoryTreeCode" clearable class="inputWidth" ></el-input>
        </el-form-item>
    ...
    <el-dialog title="修改菜单" :visible.sync="dialogMenuEdit"  width="30%" :close-on-click-modal="false" :before-close="cancelDialog">
        <el-form :model="editMenuList" :rules="rulesAddMenu" ref="editMenuList" label-width="100px">
          ...
          <el-form-item label="上级分类编码" prop="categoryId"  class="mar-top-20">
            <el-input  v-model="editMenuList.categoryId" clearable class="inputWidth" ></el-input>
          </el-form-item>
          <el-form-item label="树状查询码"   prop="categoryTreeCode"   class="mar-top-20">
            <el-input  v-model="editMenuList.categoryTreeCode" clearable class="inputWidth" ></el-input>
          </el-form-item>
          <el-form-item label="排序"   prop="sortNo"   class="mar-top-20">
  ...
  export default {
    data() {
     ...
     var validatePass = (rule, value, callback) => {
      ...
      return {
        ...
        addMenuList:{//新增菜单的数据
          categoryName:'',//名称
          categoryParentId:'001',//类型 1是目录 2是菜单
          categoryRemark:'',//备注
          sortNo:'',//排序
          categoryTreeCode:'',//树状查询码
        },
        ...
        rulesAddMenu:{//新增菜单
          ...
          categoryParentId:[
            { required: true, message: '请输入上级分类编码', trigger: 'blur' },
          ],
          sortNo: [
          ...
          categoryTreeCode: [
             {required: true,  message: '请输入树状查询码', trigger: 'blur' },
             { min: 1, max: 40, message: '长度在 1 到 40 个字符', trigger: 'blur' }
          ],
         },
        rulesAddButton:{//新增按钮
        ...
        getMenuUrl:this.$store.state.url+'neusoftMall/categoryManagement/backend/category/getCategory',//查询菜单,
        getMenuDetail:this.$store.state.url+'webauth/menu/getMenuDetail',//查询菜单详情, 
        delMenuUrl:this.$store.state.url+'neusoftMall/categoryManagement/backend/category/deleteCategory',//删除菜单, 
        addNewMenuUrl:this.$store.state.url+'neusoftMall/categoryManagement/backend/category/addCategory',//新增菜单, 
        getModifyMenuUrl:this.$store.state.url+'neusoftMall/categoryManagement/backend/category/updateCategory',//修改菜单
    ...
    methods: {
      ...
      findMenuDetail(){
        ...
        this.$axios.get(this.getMenuDetail,params).then(res =>  {//查询菜单详情
      ...
      findButtonList(){
        ...
        this.$axios.get(this.getMenuBtnList,params).then(res =>  {//查询按钮详情
            if(0==res.data.code){//查询成功
              this.buttonRadio="";
      ...
      addMenuSure(formName){
         this.$refs[formName].validate((valid) => {
          if (valid) {
              ...
              params.append('categoryParentId', this.addMenuList.categoryParentId);
              params.append('categoryTreeCode', this.addMenuList.categoryTreeCode);
      ...
      //修改菜单树
      editTree(){
        if(this.menuDetail.categoryName){
       ...
       this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            // params.append('menuCode', this.menuDetail.menuCode);
              ...
              // params.append('categoryTreeCode', this.editMenuList.categoryTreeCode);
      ...
      getData() {
        this.openFullScreen();//打开遮罩
        this.$axios.get(this.getMenuUrl).then(res =>  {
          this.loading.close(); //关闭加载
          if(0==res.data.code){//查询成功
